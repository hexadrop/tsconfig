# Contributing to @hexadrop/tsconfig

Thank you for your interest in contributing to **@hexadrop/tsconfig** — A base TSConfig for working with Node + ESM + Strictest.
...
1. **Open an issue** using the appropriate template ([Bug Report](https://github.com/hexadrop/tsconfig/issues/new?template=bug_report.yml) or [Feature Request](https://github.com/hexadrop/tsconfig/issues/new?template=feature_request.yml))
...
```
git clone https://github.com/hexadrop/tsconfig.git
cd tsconfig
```

This package ships a single `tsconfig.json`. There is no build step.
...
```
feat: narrow narrower exact-optional-properties
fix: loosen moduleResolution to bundler
docs: update contributing guide
chore(deps): bump dependencies
ci: split unit and e2e jobs
```
...
```
feat!: switch moduleResolution to node16

BREAKING CHANGE: moduleResolution is now node16 instead of bundler.
...
**Examples:** `feat/loosen-checkjs`, `fix/module-resolution`, `hotfix/patch`
...
```bash
npm install @hexadrop/tsconfig@beta
```

Snapshot releases do not modify or commit version files. They use the pending changesets to calculate a version such as `0.0.6-beta-20260802153000`. Documentation, tooling, and other changes without a changeset do not publish a beta.
...
Use [GitHub Discussions](https://github.com/hexadrop/tsconfig/discussions) — not issues — for questions, ideas, and general conversation.
